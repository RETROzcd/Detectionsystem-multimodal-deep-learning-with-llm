...
class RuleCheckModel:
    ...
    def __init__(self, model_configs: dict):
        ...
        if self._model_name in [ModelType.GPT_4O, ModelType.O4_MINI, ModelType.O3_MINI]:
            self._client = AzureOpenAI(
                    ...
                    azure_endpoint=model_configs["azure_endpoint"]
                )
    ...
    def dump_request_rules(self, request: RuleCheckRequest):
        ...
        #     for rule in request.get_rules():
        #         f.write(f"rule_id={rule.get_rule_id()}, chapter={rule.get_chapter()}, title={rule.get_title()}, method={rule.get_method()}, audit_content={rule.get_audit_content()}, preconditions={rule.get_preconditions()}, design_age_range={request.get_design_age_range()}\n")
